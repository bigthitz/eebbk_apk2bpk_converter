...
static int parse_zip(char* input, char* output, int mode, int verbose) {
    ...
    for (uint16_t i=1;i<=heo.total_central_directory_num;i++) {
        ...
        (void)pwrite(fdo, ptr + sizeof(hl)+ hds.file_name_length + hds.extra_length, hds.compressed_size + hl.extra_field_length, local_offset+sizeof(hl)+hds.file_name_length+hds.extra_length);
        ...
        if ((flag & 0x8)) {
            ...
            (void)pwrite(fdo, &hdd, sizeof(hdd), local_offset+sizeof(hl)+hds.file_name_length+hds.extra_length+hds.compressed_size);
        }
        if (mode == CFG_ENCODE) {
            xor((uint8_t*)&hl+sizeof(hl.magic), sizeof(hl)-sizeof(hl.magic)-sizeof(hl.extra_length), xorCodeLOCAL);
        }
        if (!(flag & 0x8)) {
